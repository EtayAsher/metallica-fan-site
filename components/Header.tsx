...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 bg-[#080808]/98 backdrop-blur border-b border-neutral-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-14 md:h-16">
          {/* Logo */}
          <Link href="/" className="flex items-center gap-3 shrink-0 group">
            <div className="w-1 h-6 bg-yellow-500" />
            <span className="text-white font-black text-lg tracking-[0.2em] uppercase group-hover:text-yellow-400 transition-colors duration-200">
              Metallica
            </span>
            <span className="hidden sm:block text-neutral-600 text-[10px] tracking-[0.4em] uppercase font-light border-l border-neutral-800 pl-3">
              Fan Site
            </span>
          </Link>
          ...
          <nav className="hidden md:flex items-center gap-0.5">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className={`px-3.5 py-2 text-xs tracking-[0.12em] uppercase font-medium transition-all duration-150 ${
                  pathname === link.href
                    ? "text-yellow-400 bg-yellow-500/8"
                    : "text-neutral-500 hover:text-neutral-100 hover:bg-white/4"
                }`}
              >
                {link.label}
              </Link>
